/*
 * nRF5340 **network core** UARTE0 backend for the YRM100 RFID library.
 *
 * Adapted from yrm100_uart_nrf5340.c (app core UARTE1).
 * Changes: UARTE1 → UARTE0 (0x41013000), P1 → net-core P1 (0x418C0800),
 *          ISR name → SERIAL0_Handler, IRQ 9 → 5.
 *
...
/* ── UARTE0 interrupt handler (network core SERIAL0, IRQ 5) ──────────── */
...
yrm100_nrf_uart_init(uint32_t baud)
{
    ...
    /* UARTE0 peripheral (network core) */
    NRF_UARTE0->PSEL_TXD  = YRM100_PSEL_TX;
    NRF_UARTE0->PSEL_RXD  = YRM100_PSEL_RX;
    NRF_UARTE0->PSEL_CTS  = UARTE_PSEL_DISCONNECT;
    NRF_UARTE0->PSEL_RTS  = UARTE_PSEL_DISCONNECT;
    NRF_UARTE0->BAUDRATE   = baud_to_reg(baud);
    NRF_UARTE0->CONFIG     = 0;   /* 8N1, no hardware flow control */
    NRF_UARTE0->ENABLE     = UARTE_ENABLE_ENABLED;

    /* RX: 1-byte DMA with auto-restart */
    rx_head = 0;
    rx_tail = 0;
    NRF_UARTE0->RXD_PTR    = (uint32_t)&rx_dma_buf;
    NRF_UARTE0->RXD_MAXCNT = 1;
    NRF_UARTE0->SHORTS     = UARTE_SHORTS_ENDRX_STARTRX;
    NRF_UARTE0->EVENTS_ENDRX = 0;
...
yrm100_nrf_uart_send(const uint8_t *data, uint16_t len)
{
    ...
    NRF_UARTE0->TXD_PTR    = (uint32_t)data;
...
const yrm100_uart_t yrm100_nrf5340_net_uart = {
